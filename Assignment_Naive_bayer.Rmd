...
#The purpose of this assignment is to use Naive Bayes for classification.Will be using 3 different methods to compare
#1.Easy method 
#2.Using the Naive Bayers equation 
#3.Using the Naive Bayers function in R 
...
$$P(L=1|CC=1,Onl=1) = \frac{(P(CC=1|L=1)*P(Onl=1|L=1))*P(L=1)} {(P(CC=1|L=1)*P(Onl=1|L=1))*P(L=1)+(P(CC=1|L=0)*P(Onl=1|L=0))*P(L=0)}$$
...
#We got the same exact output we receive in Previous method.i.e in question (e): because the joint and marginal probabilities we calculated in question (e)  are exactly same as given by the naive bayer's function.  
```
